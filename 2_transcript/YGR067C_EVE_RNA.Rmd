...
txi <- tximport(files, type="salmon", 
                tx2gene = tx2gene_adj[,c("tx_id", "ensgene", "symbol")], 
...
```{r}
pathfind_enrich <- myenrich(output_df_gluc)
```
...
saveWorkbook(wb, "../out/Tables/transcript_counts.xlsx")
```

```{r}
# S table 2.2 - Diff exp of genes
wb <- createWorkbook()
addWorksheet(wb, "MUT.v.REF_gluc_tra")
addWorksheet(wb, "MUT.v.REF_etoh_tra")
addWorksheet(wb, "etoh.v.gluc_REF_tra")
addWorksheet(wb, "etoh.v.gluc_MUT_tra")
writeData(wb, "MUT.v.REF_gluc_tra", sig_glucose_nice, keepNA = T)
writeData(wb, "MUT.v.REF_etoh_tra", sig_ethanol_nice, keepNA = T)
writeData(wb, "etoh.v.gluc_REF_tra",sig_WT_nice, keepNA = T)
writeData(wb, "etoh.v.gluc_MUT_tra",sig_MUT_nice, keepNA = T)
saveWorkbook(wb, "../results/Tables/sig_expression_transcripts.xlsx")
```
...
saveWorkbook(wb, "../results/Tables/exploratory_enrichment.xlsx")
```
