...
```{r}
ref1 <- merge(SMALLPOS, AvC$POS, by.x = "Alignment ID", by.y = "Alignment_ID")
ref2 <- merge(SMALLNEG, AvC$NEG, by.x = "Alignment ID", by.y = "Alignment_ID")
ref3 <- rbind(ref1,ref2)
```

```{r}
MUT1 <- merge(SMALLPOS, BvD$POS, by.x = "Alignment ID", by.y = "Alignment_ID")
MUT2 <- merge(SMALLNEG, BvD$NEG, by.x = "Alignment ID", by.y = "Alignment_ID")
MUT3 <- rbind(MUT1,MUT2)
```
...
```{r}
metab_names_ref <- get_unique_metab_names(ref3)
metab_names_MUT <- get_unique_metab_names(MUT3)
```
...
```{r}
ref_RDS50_final <- merge(all_metab %>% dplyr::select(Query, KEGG), metab_names_ref, by.x = "Query", by.y = "Metabolite name") %>% subset(KEGG %in% yeast9keggs)
MUT_RDS50_final <- merge(all_metab %>% dplyr::select(Query, KEGG), metab_names_MUT, by.x = "Query", by.y = "Metabolite name") %>% subset(KEGG %in% yeast9keggs)
```
...
saveWorkbook(wb, file = "../results/Tables/metabolite_data.xlsx")
```

```{r}
# S table 3.2 - Univariate stat analysis
wb <- createWorkbook()
addWorksheet(wb, "MUT.v.REF_gluc_met")
addWorksheet(wb, "MUT.v.REF_etoh_met")
addWorksheet(wb, "etoh.v.gluc_REF_met")
addWorksheet(wb, "etoh.v.gluc_MUT_met")
writeData(wb, "MUT.v.REF_gluc_met", log_RDS50_final %>% subset(p<0.1), keepNA = T)
writeData(wb, "MUT.v.REF_etoh_met", PDS_RDS50_final %>% subset(p<0.1), keepNA = T)
writeData(wb, "etoh.v.gluc_REF_met", ref_RDS50_final %>% subset(p<0.1), keepNA = T)
writeData(wb, "etoh.v.gluc_MUT_met", MUT_RDS50_final %>% subset(p<0.1), keepNA = T)
saveWorkbook(wb, file = "../results/Tables/sig_abundance_metabolites.xlsx")
```

```{r}
# S table 4.1 (metabolomics)

wb <- loadWorkbook(file = "../results/Tables/prediction_vs_empirical_data.xlsx")
...
saveWorkbook(wb, "../results/Tables/prediction_vs_empirical_data.xlsx", overwrite = T)
```
...
saveWorkbook(wb, "../out/Tables/exploratory_enrichment.xlsx", overwrite = T)
```
...
```{r}
set.seed(20120)
pdf(file = "../results/Figures/Fig_2D.pdf")
