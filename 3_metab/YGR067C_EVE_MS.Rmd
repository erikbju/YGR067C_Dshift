...
```{r}
grps <- unique(merged_nonorm_RDS50$Group)
cmb <- combn(grps,2)
...
for (i in 1:ncol(cmb)) {
  contrasts_RDS50[[paste0(cmb[,i], collapse = "_")]] <- merged_nonorm_RDS50[, merged_nonorm_RDS50$Group %in% cmb[,i]]
} 
```
...
```{r}
metab_names_log <- read.table(file = "./out/metab_names_log.txt", header=T)
metab_names_PDS <- read.table(file = "./out/metab_names_PDS.txt", header=T)
```
...
fig2c <- plot_grid(pvolcrna, pvolcmetab, leg, ncol = 3, rel_widths = c(1,1,0.2))
```
...
```{r}
log_RDS50_final <- merge(all_metab %>% dplyr::select(Query, KEGG), metab_names_log_RDS50, by.x = "Query", by.y = "Metabolite name") %>% subset(KEGG %in% yeast9keggs)

PDS_RDS50_final <- merge(all_metab %>% dplyr::select(Query, KEGG), metab_names_PDS_RDS50, by.x = "Query", by.y = "Metabolite name") %>% subset(KEGG %in% yeast9keggs)
```

```{r}
write.table(all_log_RDS50_final, file = "./out/log_RDS50.txt", sep = "\t",
            quote = F,row.names = F)

write.table(all_PDS_RDS50_final, file = "./out/PDS_RDS50.txt", sep = "\t",
            quote = F,row.names = F)
```

```{r}
svglite(file = "./out/fig2c.svg", width = 7, height = 8)
fig2c
dev.off()
```
...
```{r}
log_RDS50_final <- read_tsv("./out/log_RDS50.txt")
PDS_RDS50_final <- read_tsv("./out/PDS_RDS50.txt")
```